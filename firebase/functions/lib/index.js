...
const auth = getAuth();
const roles = ["super_admin", "hospital_admin", "doctor", "surgeon", "anesthetist", "nurse", "pharmacist", "pathologist", "lab_manager", "lab_technician", "radiologist", "radiology_technician", "receptionist", "mortuary_officer", "ict_admin", "records_officer", "patient"];
const otpPurposes = ["account_activation", "login_verification", "forgot_password", "sensitive_action"];
...
async function audit(actorUid, actorRole, hospitalId, action, collectionName, documentId) {
...
export const recordLoginActivity = onCall({ enforceAppCheck: true }, async (request) => {
    ...
    if (input.loginStatus === "success" && !request.auth)
        throw new HttpsError("unauthenticated", "Successful login events require authentication.");
...
export const closeLoginSession = onCall({ enforceAppCheck: true }, async (request) => {
    ...
    if (snapshot.empty)
        throw new HttpsError("not-found", "Active login session was not found.");
...
function otpDocId(userId, purpose) {
...
function govCareEmailTemplate(input) {
...
async function sendSecurityEmail(email, title, message) {
    const config = transporterConfig();
    if (!config) {
...
async function getUserContextForOtp(request, purpose, email) {
    if (purpose === "forgot_password" && !request.auth) {
        ...
        if (userSnap.exists && user.status && user.status !== "active")
            throw new HttpsError("permission-denied", "This account is not active.");
        if (!userSnap.exists)
            throw new HttpsError("not-found", "No active EHR user profile was found for this email address.");
        return {
            uid: userRecord.uid,
...
export const requestEmailOtp = onCall({ enforceAppCheck: true }, async (request) => {
    ...
    if (!user.email)
        throw new HttpsError("failed-precondition", "User email is missing.");
...
export const verifyEmailOtp = onCall({ enforceAppCheck: true }, async (request) => {
    ...
    if (!snap.exists || !data || data.status !== "pending")
        throw new HttpsError("failed-precondition", "No active OTP request was found.");
...
export const registerPatientWithGuardian = onCall(async (request) => {
    ...
    if (new Date().getUTCMonth() < dob.getUTCMonth())
        age -= 1;
    if (age >= 16 && !input.patient.nicOrPassport?.trim())
        throw new HttpsError("invalid-argument", "Adult patients require a NIC or passport.");
    if (age < 16 && (!input.patient.birthCertificateNo?.trim() || !input.guardian))
        throw new HttpsError("invalid-argument", "Children require birth certificate and guardian details.");
    ...
    if (result.guardianId)
        await audit(context.uid, String(context.token.role), input.hospitalId, "link", "guardians", result.guardianId);
...
export const ensurePatientPortalProfile = onCall(async (request) => {
    try {
        ...
        if (context.uid !== input.uid)
            throw new HttpsError("permission-denied", "Cannot create another patient's profile.");
        const userRecord = await auth.getUser(context.uid);
        const usedGoogle = userRecord.providerData.some((provider) => provider.providerId === "google.com");
        if (!usedGoogle)
            throw new HttpsError("permission-denied", "Patient portal login must use Google.");
        const userRef = db.collection("users").doc(context.uid);
        const existing = await userRef.get();
        if (existing.exists) {
            const data = existing.data();
            if (data?.role !== "patient")
                throw new HttpsError("permission-denied", "This account is not a patient account.");
...
export const generatePatientId = onCall(async (request) => {
    ...
    if (context.token.role !== "super_admin" && context.token.hospitalId !== input.hospitalId)
        throw new HttpsError("permission-denied", "Wrong hospital.");
    const counterRef = db.collection("hospitals").doc(input.hospitalId).collection("counters").doc("patients");
    const result = await db.runTransaction(async (tx) => {
        const snap = await tx.get(counterRef);
        const next = (snap.data()?.value ?? 0) + 1;
...
function evaluatePrescriptionSafety(input) {
    const warnings = [];
    ...
    for (const line of input.lines) {
        ...
        if (allergyMatch)
            warnings.push(`Allergy warning for ${line.generic}.`);
        if (line.stock <= 0)
            warnings.push(`${line.generic} is out of stock.`);
        if (line.stock > 0 && line.stock < line.quantity)
            warnings.push(`${line.generic} has partial stock.`);
    }
    for (const [generic, count] of counts.entries()) {
        if (count > 1)
            warnings.push(`Duplicate medicine detected: ${generic}.`);
    }
...
export const verifyPrescriptionSafety = onCall(async (request) => {
    ...
    if (input.prescriptionId) {
        ...
        const data = snap.data();
        if (!snap.exists || data?.hospitalId !== input.hospitalId)
...
export const generateAdminReport = onCall(async (request) => {
    ...
    if (!["super_admin", "hospital_admin"].includes(role)) {
        if (!claimDepartmentId)
            throw new HttpsError("permission-denied", "A department assignment is required for report access.");
...
export const submitMedicalDecisionRequest = onCall(async (request) => {
    ...
    if (input.patientUid !== context.uid)
        throw new HttpsError("permission-denied", "Patients can request reports only for their own account.");
    if (input.hospitalId !== String(context.token.hospitalId ?? ""))
        throw new HttpsError("permission-denied", "Hospital access mismatch.");
    if (input.patientId !== String(context.token.patientId ?? ""))
        throw new HttpsError("permission-denied", "Patient identity mismatch.");
...
export const reviewMedicalDecisionRequest = onCall(async (request) => {
    ...
    if (!requestSnap.exists)
        throw new HttpsError("not-found", "Medical report request not found.");
...
export const verifyMedicalDecisionReport = onCall(async (request) => {
    ...
    if (!reportSnap.exists)
        return { valid: false, message: "No valid released report was found." };
    const report = reportSnap.data() ?? {};
    if (report.status !== "active" || report.releaseStatus !== "released")
        return { valid: false, message: "This report is not currently valid." };
    if (report.expiryDate && String(report.expiryDate) < new Date().toISOString().slice(0, 10))
        return { valid: false, message: "This report has expired." };
    if (input.token) {
        const providedHash = crypto.createHash("sha256").update(input.token).digest("hex");
        if (providedHash !== report.verificationTokenHash)
            return { valid: false, message: "The verification token is invalid." };
    }
    await db.collection("auditLogs").add({
...
export const getPatientReportDownloadUrl = onCall(async (request) => {
    ...
    const report = reportSnap.data();
    if (report?.patientUid !== context.uid)
        throw new HttpsError("permission-denied", "Report does not belong to this patient.");
    ...
    if (!released && !ownUpload)
        throw new HttpsError("permission-denied", "This report has not been released to the patient.");
    if (!report?.storagePath)
        throw new HttpsError("failed-precondition", "Report file is not ready.");
...
export const approveAdmissionAndAssignBed = onCall(async (request) => {
    ...
    const result = await db.runTransaction(async (tx) => {
        ...
        if (admissionRequest.hospitalId !== input.hospitalId || bed.hospitalId !== input.hospitalId)
            throw new HttpsError("permission-denied", "Hospital mismatch.");
        if (bed.status !== "available")
            throw new HttpsError("failed-precondition", "Selected bed is not available.");
...
export const auditDocumentWrites = onDocumentWritten("{collectionName}/{documentId}", async (event) => {
    if (event.params.collectionName === "auditLogs")
        return;
