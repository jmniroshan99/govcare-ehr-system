...
async function main() {
  ...
  if (process.platform === "win32") {
    startProcess("postgres-api", "cmd.exe", ["/c", "npm run dev"], path.join(root, "server"));
    startProcess("frontend", "cmd.exe", ["/c", "npm run dev -- --host 127.0.0.1 --port 5300 --strictPort"]);
  } else {
    const viteBin = path.join(root, "node_modules", "vite", "bin", "vite.js");
    startProcess("postgres-api", "npm", ["run", "dev"], path.join(root, "server"));
    startProcess("frontend", process.execPath, [viteBin, "--host", "127.0.0.1", "--port", "5300", "--strictPort"]);
  }

  await waitFor("http://127.0.0.1:4001/health", "PostgreSQL API");
  const frontendReady = await waitFor("http://127.0.0.1:5300", "Frontend");
  if (frontendReady) openBrowser(frontendUrl);

  console.log("GovCare local stack requested. To stop old Node processes, close them from Task Manager or run taskkill /F /IM node.exe.");
  windowlessExit();
}
