      ...
      .map((group) => ({
        ...
        entries: group.entries
          .filter((entry) => {
            ...
            const allowed = role ? allowedRoles.includes(role) && canAccessPath(profile, entry.href) : false;
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="min-w-0 transition-[padding] duration-300 lg:pl-[82px]">
        ...
        <main className="mx-auto flex min-h-screen w-full max-w-[1480px] flex-1 flex-col overflow-y-auto overflow-x-hidden p-4 sm:p-6">{children}</main>
      </div>
    </div>
  );
}
