          ...
          <CardContent className="space-y-4">
            {!selected ? <p className="text-sm text-muted-foreground">Select a prescription to verify.</p> : <>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {selected.pharmacy_status === "verified" && <Button onClick={() => void perform(() => dispensePharmacyPrescription(selected.id, selected.lines.map((line) => ({ prescriptionItemUuid: line.id, medicineUuid: line.medicineId ?? null, quantity: line.quantity ?? 1 })), notes), "Medicines dispensed and stock updated.")}><PackageCheck className="h-4 w-4" />Dispense all</Button>}
              </div>
