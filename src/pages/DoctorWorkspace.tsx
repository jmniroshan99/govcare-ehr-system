...
export function DoctorWorkspace() {
  ...
  const [labItems, setLabItems] = useState<Array<{ testName: string; specimen: string }>>([]);
  const [radiologyType, setRadiologyType] = useState("X-ray");
  const [bodyArea, setBodyArea] = useState("Chest");
  const [radiologyItems, setRadiologyItems] = useState<Array<{ imagingType: string; bodyArea: string }>>([]);
  ...
  function addLaboratoryTest() {
    ...
    setLabItems((current) => [...current, { testName, specimen }]);
  }
  ...
  function addRadiologyStudy() {
    ...
    setRadiologyItems((current) => [...current, { imagingType, bodyArea: area }]);
  }
    ...
    <div className="space-y-5 pb-10">
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 lg:grid-cols-2">
          ...
          <div className="lg:col-span-2"><h3 className="mb-3 font-bold text-slate-900">Vital signs</h3><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><label className="space-y-1 text-sm font-semibold text-slate-700">Temperature<Input value={draft.temperature} onChange={(event) => updateDraft("temperature", event.target.value)} placeholder="36.8 °C" /></label><label className="space-y-1 text-sm font-semibold text-slate-700">Blood pressure<Input value={draft.bloodPressure} onChange={(event) => updateDraft("bloodPressure", event.target.value)} placeholder="120/80" /></label><label className="space-y-1 text-sm font-semibold text-slate-700">Pulse<Input value={draft.pulse} onChange={(event) => updateDraft("pulse", event.target.value)} placeholder="72 bpm" /></label><label className="space-y-1 text-sm font-semibold text-slate-700">Oxygen saturation<Input value={draft.oxygenSaturation} onChange={(event) => updateDraft("oxygenSaturation", event.target.value)} placeholder="98%" /></label></div></div><label className="space-y-1 text-sm font-semibold text-slate-700 lg:col-span-2"><span>Completion override reason (required only when urgent laboratory or radiology orders remain unresolved)</span><Input value={completionOverrideReason} onChange={(event) => setCompletionOverrideReason(event.target.value)} placeholder="Clinical reason for completing before urgent results are resolved" /></label>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><label className="space-y-1 text-sm font-semibold text-slate-700">Medicine<Select value={medicineDraft.medicineId ?? ""} onChange={(event) => { const option = medicineOptions.find((item) => item.id === event.target.value); setMedicineDraft((item) => ({ ...item, medicineId: option?.id ?? null, medicineName: option?.name ?? "", genericName: option?.generic_name ?? "", strength: option?.strength ?? "" })); }}><option value="">Select medicine</option>{medicineOptions.map((option) => <option key={option.id} value={option.id}>{option.name}{option.strength ? ` · ${option.strength}` : ""}{option.available_stock !== undefined ? ` · stock ${option.available_stock}` : ""}</option>)}</Select></label><label className="space-y-1 text-sm font-semibold text-slate-700">Strength<Input value={medicineDraft.strength ?? ""} onChange={(event) => setMedicineDraft((item) => ({ ...item, strength: event.target.value }))} placeholder="500 mg" /></label><label className="space-y-1 text-sm font-semibold text-slate-700">Dose<Input value={medicineDraft.dosage} onChange={(event) => setMedicineDraft((item) => ({ ...item, dosage: event.target.value }))} placeholder="1 tablet" /></label><label className="space-y-1 text-sm font-semibold text-slate-700">Route<Select value={medicineDraft.route ?? "Oral"} onChange={(event) => setMedicineDraft((item) => ({ ...item, route: event.target.value }))}><option>Oral</option><option>IV</option><option>IM</option><option>Topical</option><option>Inhaled</option></Select></label><label className="space-y-1 text-sm font-semibold text-slate-700">Frequency<Input value={medicineDraft.frequency} onChange={(event) => setMedicineDraft((item) => ({ ...item, frequency: event.target.value }))} placeholder="TDS" /></label><label className="space-y-1 text-sm font-semibold text-slate-700">Duration<Input value={medicineDraft.duration} onChange={(event) => setMedicineDraft((item) => ({ ...item, duration: event.target.value }))} placeholder="3 days" /></label><label className="space-y-1 text-sm font-semibold text-slate-700">Quantity<Input type="number" min="1" value={medicineDraft.quantity ?? 1} onChange={(event) => setMedicineDraft((item) => ({ ...item, quantity: Number(event.target.value) || 1 }))} /></label><label className="space-y-1 text-sm font-semibold text-slate-700">Instructions<Input value={medicineDraft.instructions ?? ""} onChange={(event) => setMedicineDraft((item) => ({ ...item, instructions: event.target.value }))} placeholder="After meals" /></label></div>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <Card><CardHeader><CardTitle className="flex items-center gap-2"><FlaskConical className="h-5 w-5 text-primary" />Laboratory order</CardTitle></CardHeader><CardContent className="space-y-3"><label className="block space-y-1 text-sm font-semibold text-slate-700">Test<Input value={labTest} onChange={(event) => setLabTest(event.target.value)} /></label><label className="block space-y-1 text-sm font-semibold text-slate-700">Specimen<Input value={labSpecimen} onChange={(event) => setLabSpecimen(event.target.value)} /></label><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={addLaboratoryTest}><Plus className="h-4 w-4" />Add test</Button><Button onClick={() => void orderLab()} disabled={!labItems.length}><Send className="h-4 w-4" />Send {labItems.length || ""} to Laboratory</Button></div><div className="space-y-2">{labItems.map((item, index) => <div key={`${item.testName}-${index}`} className="flex items-center justify-between rounded-md border border-border p-2 text-sm"><span><strong>{item.testName}</strong> · {item.specimen}</span><Button variant="destructive" className="min-h-9 px-3 py-1" onClick={() => setLabItems((current) => current.filter((_, itemIndex) => itemIndex !== index))}>Remove</Button></div>)}{!labItems.length && <p className="text-sm text-muted-foreground">No tests added.</p>}</div></CardContent></Card>
        <Card><CardHeader><CardTitle className="flex items-center gap-2"><ScanLine className="h-5 w-5 text-primary" />Radiology order</CardTitle></CardHeader><CardContent className="space-y-3"><label className="block space-y-1 text-sm font-semibold text-slate-700">Imaging<Select value={radiologyType} onChange={(event) => setRadiologyType(event.target.value)}><option>X-ray</option><option>CT</option><option>MRI</option><option>Ultrasound</option><option>Mammography</option></Select></label><label className="block space-y-1 text-sm font-semibold text-slate-700">Body area<Input value={bodyArea} onChange={(event) => setBodyArea(event.target.value)} /></label><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={addRadiologyStudy}><Plus className="h-4 w-4" />Add study</Button><Button onClick={() => void orderRadiology()} disabled={!radiologyItems.length}><Send className="h-4 w-4" />Send {radiologyItems.length || ""} to Radiology</Button></div><div className="space-y-2">{radiologyItems.map((item, index) => <div key={`${item.imagingType}-${item.bodyArea}-${index}`} className="flex items-center justify-between rounded-md border border-border p-2 text-sm"><span><strong>{item.imagingType}</strong> · {item.bodyArea}</span><Button variant="destructive" className="min-h-9 px-3 py-1" onClick={() => setRadiologyItems((current) => current.filter((_, itemIndex) => itemIndex !== index))}>Remove</Button></div>)}{!radiologyItems.length && <p className="text-sm text-muted-foreground">No imaging studies added.</p>}</div></CardContent></Card>
      </div>
    </div>
  );
}
