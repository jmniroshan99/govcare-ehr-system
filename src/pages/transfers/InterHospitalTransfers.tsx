...
export function InterHospitalTransfers({ direction = "all" }: { direction?: Direction }) {
  ...
  return <PageTransition><div className="space-y-5"><header className="page-hero flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-semibold text-primary">GovCare hospital network</p><h1 className="mt-1 text-2xl font-bold text-slate-950">{title}</h1><p className="mt-2 text-sm text-muted-foreground">Coordinate acceptance, receiving-bed reservation, secure document sharing, transport, departure and arrival.</p></div><Button variant="outline" onClick={() => void load()}><RefreshCw className="h-4 w-4" />Refresh</Button></header>
    {direction !== "incoming" && <Card><CardHeader><CardTitle>Create transfer draft</CardTitle></CardHeader><CardContent><form className="grid gap-4 md:grid-cols-2 xl:grid-cols-6" onSubmit={(event) => void submit(event)}><Input required value={sourceAdmissionId} onChange={(event) => setSourceAdmissionId(event.target.value)} placeholder="Source admission UUID" /><Select required value={destinationHospitalId} onChange={(event) => setDestinationHospitalId(event.target.value)}><option value="">Receiving hospital</option>{hospitals.map((hospital) => <option key={hospital.id} value={hospital.id}>{hospital.name}</option>)}</Select><Select value={priority} onChange={(event) => setPriority(event.target.value)}>{["ROUTINE","URGENT","EMERGENCY","ICU_PRIORITY","ISOLATION_PRIORITY"].map((item) => <option key={item}>{item}</option>)}</Select><Input required value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Transfer reason" /><Input required value={summary} onChange={(event) => setSummary(event.target.value)} placeholder="Clinical summary" /><Button type="submit" disabled={busy}><Plus className="h-4 w-4" />Create draft</Button></form></CardContent></Card>}
