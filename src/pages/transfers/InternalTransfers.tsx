...
export function InternalTransfers() {
  ...
  return <PageTransition><div className="space-y-5"><header className="page-hero flex flex-wrap items-start justify-between gap-4"><div><p className="text-sm font-semibold text-primary">Ward-to-ward coordination</p><h1 className="mt-1 text-2xl font-bold text-slate-950">Internal Patient Transfers</h1><p className="mt-2 text-sm text-muted-foreground">The source bed remains occupied until the destination ward confirms completion.</p></div><Button variant="outline" onClick={() => void load()}><RefreshCw className="h-4 w-4" />Refresh</Button></header>
    <Card><CardHeader><CardTitle>Request internal transfer</CardTitle></CardHeader><CardContent><form className="grid gap-4 md:grid-cols-2 xl:grid-cols-6" onSubmit={(event) => void submit(event)}><Input required value={admissionId} onChange={(event) => setAdmissionId(event.target.value)} placeholder="Admission UUID" /><Select required value={destinationWardId} onChange={(event) => setDestinationWardId(event.target.value)}><option value="">Destination ward</option>{wards.map((ward) => <option key={ward.id} value={ward.id}>{ward.wardName}</option>)}</Select><Select value={destinationBedId} onChange={(event) => setDestinationBedId(event.target.value)}><option value="">Reserve later</option>{beds.map((bed) => <option key={bed.id} value={bed.id}>{bed.bedCode}</option>)}</Select><Select value={priority} onChange={(event) => setPriority(event.target.value)}>{["ROUTINE","URGENT","EMERGENCY","ICU_PRIORITY","ISOLATION_PRIORITY"].map((item) => <option key={item}>{item}</option>)}</Select><Input required value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Transfer reason" /><Button type="submit" disabled={busy}><Plus className="h-4 w-4" />Request</Button></form></CardContent></Card>
