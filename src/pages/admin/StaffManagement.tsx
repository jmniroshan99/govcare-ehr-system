...
import { DatePicker, PhoneNumberField, SearchableSelect } from "../../components/forms";
import { GENDER_OPTIONS, TITLE_OPTIONS } from "../../data/referenceOptions";
...
function Field({ label, required = false, optional = !required, children }: { label: string; required?: boolean; optional?: boolean; children: React.ReactNode }) {
  return <label className="space-y-1.5 text-sm font-semibold text-slate-200"><FormFieldLabel required={required} optional={optional}>{label}</FormFieldLabel>{children}</label>;
}
...
function CreateStaffModal({ profileHospitalId, isSuper, options, onClose, onCreated, showToast }: { profileHospitalId: string; isSuper: boolean; options: StaffOptionsResponse; onClose: () => void; onCreated: (member: StaffMember) => void; showToast: (message: string, tone?: "success" | "info" | "warning" | "danger") => void }) {
  ...
  return <Modal title="Add New Staff" onClose={onClose} wide><form onSubmit={submit} className="space-y-5">
    <div className="rounded-md border border-cyan-800 bg-cyan-950/35 px-3 py-2 text-sm text-cyan-100">Only fields marked with <strong>*</strong> are required. Other staff details may be completed later. Account identity, hospital, role, email, and password remain required.</div>
...
function ManageStaffModal({ member, roles, hospitals, isSuper, onClose, onUpdated, showToast }: { member: StaffMember; roles: StaffRoleOption[]; hospitals: HospitalOption[]; isSuper: boolean; onClose: () => void; onUpdated: (member: StaffMember) => void; showToast: (message: string, tone?: "success" | "info" | "warning" | "danger") => void }) {
  ...
  return <Modal title={`Manage ${member.fullName}`} onClose={onClose} wide><div className="space-y-5">
    ...
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <Field label="Phone"><PhoneNumberField value={phone} onChange={setPhone} /></Field>
    ...
    <div className="grid gap-4 border-t border-slate-800 pt-5 md:grid-cols-2">
      <Field label="Change role" optional={false}><div className="flex gap-2"><Select value={role} onChange={(e) => setRole(e.target.value as StaffRole)}>{roles.map((item) => <option key={item.code} value={item.code}>{item.name}</option>)}</Select><Button disabled={saving || role === member.role} onClick={() => { if (window.confirm("Change this staff member's role? They may lose access immediately.")) void run(() => changeStaffRole(member.id, role), "Role changed successfully."); }}>Save</Button></div></Field>
      <Field label="Change department" optional={false}><div className="flex gap-2"><Select value={departmentId} onChange={(e) => setDepartmentId(e.target.value)}><option value="">Unassigned</option>{departments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select><Button disabled={saving || departmentId === (member.departmentId ?? "")} onClick={() => void run(() => changeStaffDepartment(member.id, departmentId || null), "Department changed successfully.")}>Save</Button></div></Field>
    </div>
    ...
    <div className="space-y-3 border-t border-slate-800 pt-5"><Field label="Reset temporary password" optional={false}><div className="flex gap-2"><Input value={password} onChange={(e) => setPassword(e.target.value)} /><Button variant="outline" onClick={() => setPassword(generatePassword())}>Generate</Button></div></Field><Button className="w-full" disabled={saving || password.length < 8} onClick={async () => { if (!window.confirm("Reset this staff member's password?")) return; setSaving(true); try { const result = await resetStaffPassword(member.id, password); await navigator.clipboard.writeText(result.temporaryPassword); showToast("Password reset. Temporary password copied to clipboard.", "success"); } catch (error) { showToast(error instanceof Error ? error.message : "Password reset failed.", "danger"); } finally { setSaving(false); } }}><KeyRound className="h-4 w-4" /> Reset Password</Button></div>
