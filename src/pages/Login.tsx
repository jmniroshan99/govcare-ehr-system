...
const demoStaffAccounts: Record<string, { role: Role; name: string; departmentId?: string; departmentName?: string }> = {
  "superadmin@govcare.gov.lk": { role: "super_admin", name: "Super Admin", departmentId: "ICT", departmentName: "System Administration" },
  "admin@govcare.gov.lk": { role: "hospital_admin", name: "Hospital Admin", departmentId: "Administration", departmentName: "Hospital Administration" },
  "doctor@govcare.gov.lk": { role: "doctor", name: "Dr. Anjali Perera", departmentId: "Medical OPD", departmentName: "Medical OPD" },
  "nurse@govcare.gov.lk": { role: "nurse", name: "Nurse Sanduni Perera", departmentId: "Ward 01", departmentName: "Ward Nursing" },
  "pharmacist@govcare.gov.lk": { role: "pharmacist", name: "Pharmacist Dilan Fernando", departmentId: "Pharmacy", departmentName: "Pharmacy Department" },
  "lab@govcare.gov.lk": { role: "lab_technician", name: "Lab Technician Kavindi Silva", departmentId: "Laboratory", departmentName: "Laboratory Services" },
  "radiology@govcare.gov.lk": { role: "radiologist", name: "Dr. Radiology Consultant", departmentId: "Radiology", departmentName: "Radiology Department" },
  "reception@govcare.gov.lk": { role: "receptionist", name: "Reception Officer", departmentId: "OPD", departmentName: "Out Patient Department" },
};
...
export function Login() {
  ...
  useEffect(() => {
    getGoogleRedirectUser()
      ...
      .catch((error: unknown) => setGoogleError(firebaseGoogleErrorMessage(error)));
  ...
  function enterDemoPatient(email: string) {
    clearAuth();
    window.sessionStorage.setItem("govcare-auth-mode", "demo");
    window.sessionStorage.removeItem("govcare-login-intent");
    const profile = createDemoPatientProfile(email.trim().toLowerCase(), "Demo Patient");
    setProfile(profile);
    void recordLoginActivity({ email: profile.email, loginStatus: "success", authenticationMethod: "demo", profile });
    navigate("/portal", { replace: true });
  }

  async function onGooglePatientLogin() {
    setLoginError("");
    setGoogleError("");
    clearAuth();
  ...
  async function verifyLoginOtp() {
    ...
    try {
      ...
      window.sessionStorage.removeItem("govcare-login-intent");
      navigate(defaultHomeForRole(pendingProfile.role), { replace: true });
    } catch (error) {
  ...
  async function createPatientAccount() {
    ...
    try {
      ...
      const profile = await ensurePatientPortalProfile({
        uid: user.uid,
        email: user.email ?? signupValues.email,
      ...
      console.warn("Patient signup failed; using local demo patient account.", error);
      const profile = createDemoPatientProfile(signupValues.email, signupValues.name);
      window.sessionStorage.setItem("govcare-auth-mode", "demo");
      window.sessionStorage.removeItem("govcare-login-intent");
      setProfile(profile);
      navigate("/portal", { replace: true });
    ...
    try {
      ...
      if (code === "functions/internal" || code === "functions/unavailable" || code === "functions/failed-precondition") {
        try {
          ...
          setSignupMessage(`${t("passwordReset.emailSent")} ${t("otp.gmailFallback", "OTP email service is not ready, so Firebase sent a secure reset link instead.")}`);
          return;
        } catch (fallbackError) {
    ...
    <main className="grid min-h-screen bg-white lg:grid-cols-[1.1fr_0.9fr]">
      <section className="clinical-grid relative flex flex-col justify-between overflow-hidden bg-primary p-8 text-white">
        ...
        <motion.div className="relative max-w-2xl py-16" initial={{ opacity: 0, y: 24, filter: "blur(8px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} transition={{ delay: 0.08, duration: 0.52, ease: [0.16, 1, 0.3, 1] }}>
          <p className="mb-4 inline-flex rounded-full bg-white/15 px-3 py-1 text-sm font-semibold">{t("login.firebaseCoordination")}</p>
        ...
        <Card className="professional-surface glass-panel w-full">
          ...
          <CardContent>
            ...
            {accountMode === "login" && <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
              ...
              <button className="w-full text-center text-sm font-semibold text-primary hover:underline" type="button" onClick={() => changeAccountMode("reset")}>{t("passwordReset.forgotPassword")}</button>
              ...
              <Button type="button" variant="outline" className="w-full" onClick={onGooglePatientLogin}>{t("login.patientGoogleLogin")}</Button>
            ...
            {accountMode === "patient" && <div className="space-y-3">
              ...
              <Button type="button" variant="outline" className="w-full" onClick={onGooglePatientLogin}>{t("login.createLoginGoogle")}</Button>
...
function createDemoStaffProfile(email: string, account: { role: Role; name: string; departmentId?: string; departmentName?: string }, uid = `demo-${account.role}`) {
...
function createDemoPatientProfile(email: string, displayName: string, uid = `demo-patient-${Date.now()}`) {
  const now = new Date().toISOString();
  return {
    id: uid,
    uid,
...
function isDemoPatientEmail(email: string) {
  const normalized = email.trim().toLowerCase();
  return normalized === "patient@govcare.gov.lk" || normalized === "patinet@govcare.gov.lk" || normalized.startsWith("patient@");
}

function isDemoStaffLogin(email: string, password: string) {
  return password === demoPassword && email.trim().toLowerCase() in demoStaffAccounts;
}
...
function firebaseGoogleErrorMessage(error: unknown) {
  const code = getFirebaseCode(error);
  const message = getFirebaseMessage(error);
  ...
  if (code === "auth/unauthorized-domain") return "Firebase rejected this domain. Add localhost and 127.0.0.1 in Firebase Authentication > Settings > Authorized domains.";
  if (code === "auth/operation-not-allowed") return "Google provider is disabled. Enable Google in Firebase Authentication > Sign-in method.";
  if (code === "auth/invalid-credential") return "Google sign-in credential expired or was rejected. Close any old Google popup, select Patient login with Google again, and make sure Google provider is enabled in Firebase Authentication.";
  if (code === "auth/invalid-api-key") return "Firebase API key is invalid. Check the values in .env and restart Vite.";
  if (code.startsWith("functions/")) return `Google sign-in worked, but the secure patient setup or email OTP service failed (${code})${message ? `: ${message}` : ". Deploy the latest Cloud Functions, configure App Check/SMTP, and check Firebase logs."}`;
  return `Google sign-in failed${code ? ` (${code})` : ""}${message ? `: ${message}` : ". Check Google provider and authorized domains in Firebase."}`;
}
