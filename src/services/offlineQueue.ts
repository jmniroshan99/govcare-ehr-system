...
export function isOfflineCapableNetworkError(error: unknown) {
  ...
  return message.includes("network") || message.includes("failed to fetch") || message.includes("timeout") || message.includes("unavailable");
}
...
async function requestBackgroundSync() {
  // No service worker is registered in the PostgreSQL edition. The online event
  // below performs the retry without retaining any legacy cloud worker/cache.
  return;
}
