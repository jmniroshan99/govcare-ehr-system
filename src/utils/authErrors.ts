...
export function getPostgresAuthCode(error: unknown) {
  return typeof error === "object" && error !== null && "code" in error ? String((error as { code?: unknown }).code) : "";
}

export function getPostgresAuthMessage(error: unknown) {
...
export function friendlyAuthError(error: unknown, t: TFunction) {
  const code = getPostgresAuthCode(error);
  const fallback = getPostgresAuthMessage(error) || t("authErrors.generic");
  const keyByCode: Record<string, string> = {
    ...
    "network/unavailable": "authErrors.network",
    "http/401": "authErrors.invalidCredential",
    "http/429": "authErrors.tooManyAttempts",
    "http/503": "authErrors.network",
