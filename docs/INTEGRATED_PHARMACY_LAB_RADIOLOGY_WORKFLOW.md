...
1. Doctor signs e-prescription through a Cloud Function.
...
6. Cloud Function deducts `medicineStock`, writes `medicineIssues`, generates `pharmacyReceipts`, updates prescription status, writes audit logs, and notifies patient/doctor.
...
- Use TanStack Query for lists and details, with cache invalidation after Cloud Function mutations.
...
- Use Cloud Functions for receipt generation, report approval, stock deduction, critical alert dispatch, and audit logging.
