...
For production, replace these headers with JWT authentication or Firebase token verification in Spring Security.
...
1. Replace `src/services/mediaService.ts` Firebase Storage calls with Spring API calls.
2. Add JWT or Firebase token verification in Spring Boot.
