...
-- Default PostgreSQL patient portal account used by the login page.
insert into app_users (auth_uid, hospital_id, role, full_name, email, permissions, mfa_enabled, status)
select 'seed-patient-portal', id, 'patient', 'Demo Patient', 'patient@govcare.gov.lk',
  '["portal:read","profile:self","reports:self","appointments:self"]'::jsonb, false, 'active'
from hospitals
where status = 'active'
order by created_at
limit 1
on conflict (email) do update set full_name = excluded.full_name, role = excluded.role, status = 'active', updated_at = now();
