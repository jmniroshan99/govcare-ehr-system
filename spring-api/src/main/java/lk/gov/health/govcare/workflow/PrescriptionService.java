...
public class PrescriptionService {
    ...
    public static final String SELECT="""
      ...
      p.date_of_birth,p.age_years,p.gender::text,p.blood_group,p.phone,p.allergies,u.full_name as doctor_name,ph.full_name as pharmacist_name,
    ...
    @Transactional public Map<String,Object> verify(GovCarePrincipal a,String id,Map<String,Object>in){String status=req(in,"status");if(!Set.of("verified","rejected").contains(status))throw ApiException.badRequest("Verification status must be verified or rejected.");Map<String,Object>rx=get(a,id);if(clean(rx.get("digital_signature"))==null)throw ApiException.conflict("An unsigned prescription cannot be verified.");sql.update("update prescriptions set pharmacy_status=:status,workflow_status=:status,pharmacy_verified_by=:actor,pharmacy_verified_at=now(),pharmacy_verification_notes=:notes,updated_by=:actor where id=cast(:id as uuid) and hospital_id=:hospital",map("status",status,"actor",a.id(),"notes",clean(in.get("notes")),"id",id,"hospital",a.hospitalId()));sql.update("insert into pharmacy_verifications(hospital_id,prescription_id,pharmacist_id,verification_status,notes) values(:hospital,cast(:id as uuid),:actor,:status,:notes)",map("hospital",a.hospitalId(),"id",id,"actor",a.id(),"status",status,"notes",clean(in.get("notes"))));return get(a,id);}
